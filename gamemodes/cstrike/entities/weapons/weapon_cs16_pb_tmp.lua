...
if CLIENT then
	...
	killicons_new.Add("weapon_cs16_pb_tmp", "cs/sprites/tmp_killicon", Color(255, 255, 255, 255))
end
...
SWEP.VModel 			= Model("models/weapons/cs16/paintball/v_pb_tmp.mdl")
...
function SWEP:PrimaryAttack()
	if !self.Owner:IsOnGround() then
		self:TMPFire(0.1 * self:GetAccuracy(), 0.07)
	else
		self:TMPFire(0.04 * self:GetAccuracy(), 0.07)
	end
end
...
function SWEP:TMPFire(flSpread, flCycleTime)
	...
	if self:GetAccuracy() > 1.5 then
		self:SetAccuracy(1.5)
	end
	...
	self.Owner:FireBullets3(self, self.Owner:GetShootPos(), self.Owner:EyeAngles() + self.Owner:CS16_GetViewPunch(CLIENT), flSpread, CS16_TMP_DISTANCE, CS16_TMP_PENETRATION, "CS16_PBAMMO", CS16_TMP_DAMAGE, CS16_TMP_RANGE_MODIFER, self.Owner, true, self.Owner:EntIndex())
