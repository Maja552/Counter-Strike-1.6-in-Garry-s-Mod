...
if CLIENT then
	...
	killicons_new.Add("weapon_cs16_pb_glock18", "cs/sprites/glock_killicon", Color(255, 255, 255, 255))
end
...
SWEP.VModel 			= Model("models/weapons/cs16/paintball/v_pb_glock.mdl")
...
function SWEP:SecondaryAttack()
	...
	if self:GetBurstMode() then
		if self.Owner.OldPrintMessage then
			...
			self.Owner:PrintMessage("Switched to semi-automatic")
		end
		self:SetBurstMode(false)
	else
		if self.Owner.OldPrintMessage then
			...
			self.Owner:PrintMessage("Switched to Burst-Fire mode")
		end
