...
if CLIENT then
	...
	killicons_new.Add("weapon_cs16_scout", "cs/sprites/scout_killicon", Color(255, 255, 255, 255))
end
...
--SWEP.PModel				= Model("models/weapons/cs16/p_scout.mdl")
SWEP.PModel				= Model("models/cs/p_scout.mdl")
SWEP.WModel				= Model("models/cs16/w_scout.mdl")
SWEP.VModel				= Model("models/weapons/cs16/v_scout.mdl")
--SWEP.VModel				= Model("models/cs/v_scout.mdl")

SWEP.ViewModel			= SWEP.VModel
SWEP.WorldModel			= SWEP.PModel

SWEP.Primary.Sound			= Sound("OldScout.Shot1")
...
SWEP.MaxSpeed 				= CS16_SCOUT_MAX_SPEED
SWEP.Price 					= 2750

if !gmod.GetGamemode().IsCStrike then
	SWEP.Slot			= 2
	SWEP.SlotPos		= 0
	SWEP.PModel			= Model("models/weapons/cs16/player/p_scout.mdl")
end
...
function SWEP:SCOUTFire(flSpread, flCycleTime)
	...
	self:CS16_MuzzleFlash(21, 30)
	self.Owner:SetAnimation(PLAYER_ATTACK1)

	self.Owner:FireBullets3(self, self.Owner:GetShootPos(), self.Owner:EyeAngles() + self.Owner:CS16_GetViewPunch(), flSpread, CS16_SCOUT_DISTANCE, CS16_SCOUT_PENETRATION, "CS16_762NATO", CS16_SCOUT_DAMAGE, CS16_SCOUT_RANGE_MODIFER, self.Owner, true, self.Owner:EntIndex())

	self:EmitSound(self.Primary.Sound)

	self:SetEjectBrass(CurTime() + 0.56)
...
function SWEP:Reload()
	...
	if self:CS16_DefaultReload(CS16_SCOUT_MAX_CLIP, ACT_VM_RELOAD, CS16_SCOUT_RELOAD_TIME) then
