...
function ENT:Initialize()
	self:SetNWBool("Detonated", false)
	self:SetMoveType(MOVETYPE_FLYGRAVITY)
	self:SetSolid(SOLID_BBOX)
	self:AddSolidFlags(FSOLID_NOT_STANDABLE)
	self:SetModel("models/cs16/w_smokegrenade.mdl")
...
function ENT:Beam()
	...
	effects.BeamRingPoint(self:GetPos(), 0.2, 12, radius, width, 0, Color(191, 255, 241, 32), {
		speed = 0,
		spread = 0,
		delay = 0,
		framerate = 2,
	...
	-- Shockring
	effects.BeamRingPoint(self:GetPos(), 0.5, 12, radius, width, 0, Color(191, 255, 241, 64), {
		speed = 0,
		spread = 0,
		delay = 0,
		framerate = 2,
...
function ENT:Explode(m_pTrace)
	...
	self:EmitSound(GetCS16Sound("GRENADE_FLARE"), 400, 100, 1, CHAN_ITEM)
...
function ENT:Think()
	...
	if self.dmgtime <= CurTime() and !self.m_bDetonated and self:IsOnGround() then
